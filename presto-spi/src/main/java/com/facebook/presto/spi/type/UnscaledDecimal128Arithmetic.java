...
public final class UnscaledDecimal128Arithmetic
{
    ...
    private static final long FULLBITS_64 = 0xFFFFFFFFFFFFFFFFL;
    /**
     ...
     * 10^9 fits in 2^31.
     */
    private static final int MAX_POWER_TEN_INT = 9;
    /**
     * 10^18 fits in 2^63.
     */
    private static final int MAX_POWER_TEN_LONG = 18;
    /**
     * 10^x. All unsigned values.
     */
    private static final int[] POWER_TENS_INT = new int[MAX_POWER_TEN_INT + 1];
    ...
    static {
        ...
        POWER_TENS_INT[0] = 1;
        for (int i = 1; i < POWER_TENS_INT.length; ++i) {
            POWER_TENS_INT[i] = POWER_TENS_INT[i - 1] * 10;
        }
    ...
    private static void scaleDownRoundUp(Slice decimal, int scaleFactor, Slice result)
    {
        ...
        long lo = getLong(decimal, 0);
        long hi = getLong(decimal, 1);
        if (scaleFactor <= MAX_POWER_TEN_LONG && hi == 0 && lo >= 0) {
        ...
        // TODO: use multi-precision division for rescale factors that require multiple scaleDownFive loop iterations
        if ((scaleFactor - 1) / MAX_POWER_FIVE_INT < (scaleFactor - 1) / MAX_POWER_TEN_INT) {
     ...
     * method rounds-up, eg 44/10=4, 44/5=5.
     */
    private static void scaleDownTenRoundUp(Slice decimal, int tenScale, Slice result)
    {
        boolean round;
        while (true) {
            int powerTen = Math.min(tenScale, MAX_POWER_TEN_INT);
            tenScale -= powerTen;

            int divisor = POWER_TENS_INT[powerTen];
    ...
    private static void incrementUnsafe(Slice decimal)
    {
        ...
        long hi = getLong(decimal, 1);
        setNegative(decimal, hi + 1, isNegative(decimal));
    }
    ...
    private static void setNegative(Slice decimal, long hi, boolean negative)
    {
    ...
    private static void pack(Slice decimal, int v0, int v1, long hi, boolean negative)
    {
        setRawInt(decimal, 0, v0);
        setRawInt(decimal, 1, v1);
        setNegative(decimal, hi, negative);
    }
