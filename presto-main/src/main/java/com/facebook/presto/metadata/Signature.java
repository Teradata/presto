...
public final class Signature
{
    ...
    private static boolean matchAndBind(Map<String, Type> boundParameters, Map<String, TypeParameterRequirement> typeParameters, TypeSignature parameter, Type type, boolean allowCoercion, TypeManager typeManager)
    {
        ...
        if (!parametersMatched && !typeSignature.getBase().equals(parameter.getBase())) {
            ...
            if (allowCoercion && canCoerce(typeSignature.getBase(), parameter.getBase())) {
                return matchAndBind(
                        boundParameters,
                        typeParameters,
                        parameter,
            ...
            TypeManager typeManager)
    {
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            if (componentParameter.getLiteralCalculation().isPresent()) {
                if (!typeSignatureParameter.getLongLiteral().isPresent()) {
                    return false;
                }
            }
            else if (componentParameter.getLongLiteral().isPresent()) {
                if (!typeSignatureParameter.getLongLiteral().isPresent()) {
                    return false;
                }
                if (componentParameter.getLongLiteral().get().longValue() != typeSignatureParameter.getLongLiteral().get().longValue()) {
                    return false;
                }
            }
            else {
                TypeSignature componentSignature;
                if (componentParameter.getTypeSignature().isPresent()) {
                    if (!actualTypeParameter.getTypeSignature().isPresent() && !actualTypeParameter.getNamedTypeSignature().isPresent()) {
                        return false;
                    }
                    componentSignature = componentParameter.getTypeSignature().get();
                }
                else if (componentParameter.getNamedTypeSignature().isPresent() && actualTypeParameter.getNamedTypeSignature().isPresent()) {
                    componentSignature = componentParameter.getNamedTypeSignature().get().getTypeSignature();
                }
                else {
