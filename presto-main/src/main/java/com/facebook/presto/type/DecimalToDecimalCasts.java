...
import static com.facebook.presto.util.DecimalUtils.TEN_TO_NTH_SHORT;
import static com.facebook.presto.util.DecimalUtils.doesFitPrecision;
...
public final class DecimalToDecimalCasts
{
    ...
    public static final ParametricFunction DECIMAL_TO_DECIMAL_CAST = ParametricFunction.builder(DecimalToDecimalCasts.class)
            ...
            .extraParameters(shortToShortExtraParameters())
            .methods("shortToLongCast", "longToShortCast", "longToLongCast")
            .extraParameters(precisionAndScaleExtraParameters())
            .build();
            ...
            long rescale, long halfRescale)
    {
        ...
        if (!doesFitPrecision(returnValue, (int) resultPrecision)) {
            throwCastException(value, sourcePrecision, sourceScale, resultPrecision, resultScale);
        }
        return returnValue;
    }

    private static ExtraParametersFunction shortToShortExtraParameters()
    {
        return (typeMappings, literalMappings) -> {
            DecimalType argumentType = (DecimalType) typeMappings.get("X");
            DecimalType resultType = (DecimalType) typeMappings.get("Y");
            long rescale = TEN_TO_NTH_SHORT[Math.abs(resultType.getScale() - argumentType.getScale())];
    ...
    private static ExtraParametersFunction precisionAndScaleExtraParameters()
    {
        return (typeMappings, literalMappings) -> {
            DecimalType argumentType = (DecimalType) typeMappings.get("X");
            DecimalType resultType = (DecimalType) typeMappings.get("Y");
    ...
    {
        throw new PrestoException(INVALID_CAST_ARGUMENT, format("Cannot cast DECIMAL '%s' to DECIMAL(%s, %s)",
                ShortDecimalType.toString(value, (int) sourcePrecision, (int) sourceScale),
                (int) resultPrecision, (int) resultScale));
    }
