        ...
        implements ParametricType
{
    ...
    public Type createType(List<TypeParameter> parameters)
    {
        ...
        checkArgument(
                parameters.stream().allMatch(parameter -> parameter.getNamedType().isPresent()),
                "Expected only named types as a parameters, got %s",
                parameters);
        List<NamedType> namedTypes = parameters.stream().map(parameter -> parameter.getNamedType().get()).collect(toList());
