...
public class TestMemoryPools
{
    ...
    private static final long ALMOST_TEN_MEGABYTES = TEN_MEGABYTES.toBytes() - 1;
    ...
    public void setUp()
    {
        ...
        pool = new MemoryPool(new MemoryPoolId("test"), TEN_MEGABYTES);
        fakeQueryId = new QueryId("fake");
        MemoryPool systemPool = new MemoryPool(new MemoryPoolId("testSystem"), TEN_MEGABYTES);

        queryContext = new QueryContext(new QueryId("query"), TEN_MEGABYTES, pool, systemPool, localQueryRunner.getExecutor());
        // discard all output
    ...
    public void testBlocking()
            throws Exception
    {
        assertTrue(pool.tryReserve(fakeQueryId, ALMOST_TEN_MEGABYTES));

        runDriversUntilBlock(drivers);

        assertTrue(pool.getFreeBytes() <= 0);

        pool.free(fakeQueryId, ALMOST_TEN_MEGABYTES);
